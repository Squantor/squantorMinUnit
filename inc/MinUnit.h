 ...
 */
#define MINUNIT_SETUP(name) void name(minunitState *testResults)
 ...
 */
#define MINUNIT_TEARDOWN(name) void name(minunitState *testResults)
 ...
 *
 * Used by minunit to report failing tests or just print progress. This is a weak function
 * to be overridden by the user if he wants a different report message
 *
 ...
 *
 * When a minunit check fails, this function is called. Useful for examining the callstack in embedded
 * targets to see where it fails.
 *
 */
 ...
 */
#define minUnitPass() MU__SAFE_BLOCK(testResults->checks++;)
